...
func testLoginWorking(baseURL string, userData *user) (*user, error) {
	...
	if err != nil {
		fmt.Println("[AUTH]: POST\t/login\t-> Fail\tCan't call " + baseURL + "login")
		return nil, err
	}
	// Check returned http code
	if res.StatusCode != http.StatusOK {
		errorString := fmt.Sprintf("[AUTH]: POST\t/login\t-> Fail\t[Expected: %d,\tGot: %d]", http.StatusOK, res.StatusCode)
	...
	fmt.Println("[AUTH]: POST\t/login\t\t-> Success")
	return &bodyData, nil
}

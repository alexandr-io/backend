...
// consumeLibraryUploadAuthorizationResponseMessages consume all the messages coming to the `library.upload.allowed.response` topic.
func consumeLibraryUploadAuthorizationResponseMessages() {
	// Create new consumer
	...
	// Subscribe consumer to topic library.upload.allowed.response
	...
	for {
		...
		if err == nil {
			...
			requestChannelInterface, ok := producers.LibraryUploadAuthorizationRequestChannels.Load(string(msg.Key))
			if !ok {
